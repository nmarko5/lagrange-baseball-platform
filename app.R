...
# Roster UI initializes only after cached season architecture is ready.
# ==================================================
...
server <- function(input, output, session) {
  ...
  seed_current_roster_memberships <- function(season_id=NULL){
    tryCatch({
      ...
      if(is.null(season_id)||!nzchar(as.character(season_id))){
        season_id<-isolate(input$roster_manage_season)
      }
      if(is.null(season_id)||!nzchar(as.character(season_id))){
        s<-seasons_directory()
        if(is.null(s)||nrow(s)==0)return(FALSE)
        season_id<-as.character(s$Season_ID[1])
      }
      season_id<-as.character(season_id)
      
      players<-player_lookup()
      if(is.null(players)||nrow(players)==0||!"Player_ID"%in%names(players))return(FALSE)
      ...
      if(!is.null(rm)&&nrow(rm)>0&&all(c("Organization_ID","Season_ID","Player_ID")%in%names(rm))){
  ...
  output$season_period_status<-renderUI({
    ...
    # V57 fallback: if this worker did not see an exact-season subscription row,
    # use any cached organization subscription with a configured status.
    if(access=="Not configured"&&!is.null(sub)&&nrow(sub)>0&&"Access_Status"%in%names(sub)){
      fallback<-trimws(as.character(sub$Access_Status))
      fallback<-fallback[!is.na(fallback)&fallback!=""]
  ...
  observeEvent(input$roster_sync_current,{
    tryCatch({
      seed_current_roster_memberships(input$roster_manage_season)
  ...
  # ==================================================
  # V62.3 — HITTING-ONLY ENTITLEMENTS + ROBUST SHEET REPAIR
  # ==================================================
  # Georgia State is the first module-tier test: Will Maddox receives the
  # Hitting package. Hitter Reports, Leaderboards, and Team Reports stay on;
  # Pitching and Bullpen are hidden because Georgia State supplied hitters only.
  #
  # The first external import also revealed a googlesheets4 edge case where
  # sheet_add() successfully creates a sheet but throws while returning the
  # response. V62.3 treats that as success when the sheet can be re-read,
  # repairs missing/blank headers, and safely continues the upsert import.
  
  v623_sheet_headers_match <- function(sheet_name,headers){
    tryCatch({
      last_col <- if(length(headers)<=26) LETTERS[length(headers)] else stop("Too many onboarding header columns.")
      x <- as.data.frame(googlesheets4::range_read(
        ss=SHEET_URL, sheet=sheet_name,
        range=paste0("A1:",last_col,"1"),
        col_names=FALSE, col_types="c"
      ),stringsAsFactors=FALSE)
      if(nrow(x)<1 || ncol(x)<length(headers)) return(FALSE)
      vals <- trimws(as.character(x[1,seq_along(headers),drop=TRUE]))
      identical(vals,headers)
    },error=function(e) FALSE)
  }
  
  # Override the V62.2 helper. This version is safe when sheet_add() creates
  # the sheet but errors afterward, and it always repairs a bad header row.
  v622_create_sheet_with_headers <- function(sheet_name,headers){
    if(!v622_sheet_exists(sheet_name)){
      add_error <- NULL
      tryCatch(
        googlesheets4::sheet_add(SHEET_URL,sheet_name),
        error=function(e) add_error <<- e
      )
      if(!v622_sheet_exists(sheet_name)){
        if(!is.null(add_error)) stop(add_error)
        stop(paste0("Could not create backend sheet: ",sheet_name))
      }
    }
    
    # Existing-but-blank sheets (like Entitlements after the first GSU attempt)
    # are repaired here before any range_read(..., col_names=TRUE) call.
    if(!v623_sheet_headers_match(sheet_name,headers)){
      header <- as.data.frame(
        as.list(setNames(headers,headers)),
        stringsAsFactors=FALSE, check.names=FALSE
      )
      last_col <- if(length(headers)<=26) LETTERS[length(headers)] else stop("Too many onboarding header columns.")
      googlesheets4::range_write(
        ss=SHEET_URL, data=header, sheet=sheet_name,
        range=paste0("A1:",last_col,"1"), col_names=FALSE
      )
    }
    invisible(TRUE)
  }
  
  # Explicit feature package for newly onboarded organizations. Georgia State
  # requested the Hitting package, making it our first real pricing-tier/feature-flag test.
  v62_default_entitlements <- function(org_id,season_id=""){
    keys <- unname(entitlement_features)
    is_gsu <- identical(trimws(org_id),"GEORGIA_STATE")
    enabled <- if(is_gsu) keys %in% c("HITTING","LEADERBOARDS","TEAM_REPORTS") else rep(TRUE,length(keys))
    data.frame(
      Entitlement_ID=paste0(org_id,"_ENT_",keys),
      Organization_ID=org_id,
      Season_ID=season_id,
      Feature_Key=keys,
      Enabled=ifelse(enabled,"TRUE","FALSE"),
      Package=if(is_gsu) "Hitting Trial" else "Complete Trial",
      Notes=if(is_gsu)
        "Georgia State pilot — Hitting package (no Pitching/Bullpen)"
      else
        "V62 default entitlement",
      stringsAsFactors=FALSE
    )
  }
  
  # Bundle the Georgia State logo directly into V62.3 so the first external
  # organization can brand correctly without requiring an external image host.
  v623_georgia_state_logo <- "data:image/png;base64,"
  v623_current_organization_row_base <- current_organization_row
  current_organization_row <- reactive({
    o <- v623_current_organization_row_base()
    if(nrow(o)>0 && identical(trimws(v60_chr(o$Organization_ID,"")),"GEORGIA_STATE")){
      if(!nzchar(v60_chr(o$Logo_URL,""))) o$Logo_URL <- v623_georgia_state_logo
    }
    o
  })
